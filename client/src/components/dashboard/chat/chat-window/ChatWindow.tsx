    ...
    >
      ...
      <div
        className={`flex flex-col h-full transition-all duration-300 ${
          selectedChatMenueOption
            ? "w-0 opacity-0 pointer-events-none md:w-1/2 md:opacity-100 md:pointer-events-auto lg:w-1/2 xl:w-1/2 flex-1"
            : "w-full flex-1"
        }`}
      >
        {/* Chat Header */}
      ...
      {/* ─── YAHAN BHI UPDATE KIYA HAI: Side panel responsive adjustment ─── */}
      {selectedChatMenueOption === "contact_info" && (
        <div className="w-full md:w-1/2 lg:w-1/2 xl:w-1/2 h-full shrink-0">
          <ChatDetailsPanel />
        </div>
      )}
    </div>
